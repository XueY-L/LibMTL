'''
input加权后的模型字典，输出forward结果，并能做反向传播
'''
...
class Bottleneck(nn.Module):
    ...
    def forward(self, x: Tensor, target_param=None, conv_module_name=None, bn_module_name=None) -> Tensor:
        identity = x
        ...
        if target_param == None:
            ...
            out = F.conv2d(input=x, weight=target_param[conv_module_name[0]+'.weight'], stride=self.conv1.stride, padding=self.conv1.padding)
            out = F.batch_norm(
                input=out, 
                running_mean=target_param[bn_module_name[0]+'.running_mean'], 
                running_var=target_param[bn_module_name[0]+'.running_var'], 
                weight=target_param[bn_module_name[0]+'.weight'], 
                bias=target_param[bn_module_name[0]+'.bias']
            )
            out = self.relu(out)

            out = F.conv2d(input=out, weight=target_param[conv_module_name[1]+'.weight'], stride=self.conv2.stride, padding=self.conv2.padding)
            out = F.batch_norm(
                input=out, 
                running_mean=target_param[bn_module_name[1]+'.running_mean'], 
                running_var=target_param[bn_module_name[1]+'.running_var'], 
                weight=target_param[bn_module_name[1]+'.weight'], 
                bias=target_param[bn_module_name[1]+'.bias']
            )
            out = self.relu(out)

            out = F.conv2d(input=out, weight=target_param[conv_module_name[2]+'.weight'], stride=self.conv3.stride, padding=self.conv3.padding)
            out = F.batch_norm(
                input=out, 
                running_mean=target_param[bn_module_name[2]+'.running_mean'], 
                running_var=target_param[bn_module_name[2]+'.running_var'], 
                weight=target_param[bn_module_name[2]+'.weight'], 
                bias=target_param[bn_module_name[2]+'.bias']
            )
            if self.downsample is not None:
                identity = F.conv2d(input=x, weight=target_param[conv_module_name[3]+'.weight'], stride=self.downsample[0].stride, padding=self.downsample[0].padding)
                identity = F.batch_norm(
                    input=identity, 
                    running_mean=target_param[bn_module_name[3]+'.running_mean'], 
                    running_var=target_param[bn_module_name[3]+'.running_var'], 
                    weight=target_param[bn_module_name[3]+'.weight'], 
                    bias=target_param[bn_module_name[3]+'.bias']
                )
            out = out + identity
            out = self.relu(out)
...
class ResNet(nn.Module):
    ...
    def _forward_impl(self, x: Tensor, target_param) -> Tensor:
        '''
        fuse all parameters
        '''
        # print(nets.weights)
        ...
        # x = self.conv1(x)
        x = F.conv2d(input=x, weight=target_param['conv1.weight'], stride=self.conv1.stride, padding=self.conv1.padding)
        
        # x = self.bn1(x)
        x = F.batch_norm(
            input=x, 
            running_mean=target_param['bn1.running_mean'], 
            running_var=target_param['bn1.running_var'], 
            weight=target_param['bn1.weight'], 
            bias=target_param['bn1.bias']
        )
        ...
        x = torch.flatten(x, 1)
        # x = self.fc(x)
        x = F.linear(input=x, weight=target_param['fc.weight'], bias=target_param['fc.bias'])

        return x

    def forward(self, x: Tensor, target_param) -> Tensor:
        return self._forward_impl(x, target_param)
